...
deg <- read.csv("downbyhil1_chrom.csv")
library(GenomicRanges)

targets_gr <- GRanges(
  seqnames = deg$Chromosome,
  ranges = IRanges(
    start = deg$Start,
    end = deg$End
  ),
...
ggplot(deg,
  ...
  geom_point(size = 3) +
  theme_classic()
ggsave("downbyhil1_chrom.pdf",
       width = 14,
       height = 6)

library(regioneR)

meanDistance <- function(A, ...) {
  
  hits <- distanceToNearest(A)
  
  mean(mcols(hits)$distance)
}
...
#background as detected genes:
bg <- read.csv("detected_genes.csv")  # or all genes that passed filtering
library(biomaRt)
...
colnames(map) <- c("symbol", "chr", "start", "end")
bg_map <- map[map$symbol %in% bg$symbol, ]


bg_gr <- GRanges(
  seqnames = bg_map$chr,
  ranges = IRanges(bg_map$start, bg_map$end),
  gene = bg_map$symbol
)
...
pt <- permTest(
  A = targets_gr,
  ntimes = 1000,
  randomize.function = function(A, ...) {
    sample(bg_gr, length(A), replace = FALSE)
  },
  evaluate.function = meanDistance
)
...
#activated by hil-1
new_deg <- read.csv("activatedbyhil1.csv", header = TRUE, stringsAsFactors = FALSE)
new_deg <- as.data.frame(new_deg)
new_deg <- new_deg
new_deg %in% bg_gr$gene
new_map <- map[map$symbol %in% new_deg, ]

targets_gr <- GRanges(
  seqnames = new_map$chr,
  ranges = IRanges(new_map$start, new_map$end),
  gene = new_map$symbol
)

mean(mcols(targets_gr)$gene %in% mcols(bg_gr)$gene)
